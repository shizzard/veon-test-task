...
handle_1_validate(Document, Req, State) ->
    case veon_pdu_movie_retrieve_req:validate(Document) of
        {ok, Document1} ->
            {ok, Record} = veon_pdu_movie_retrieve_req:from_document(Document1),
            handle_2_retrieve(Record, Req, State);
        {error, Errors} ->
            lager:error("Error validating movie retrieve request: ~p", [Errors]),
            {ok, Req1} = veon_web:bad_request(Req),
            {ok, Req1, State}
    end.


-spec handle_2_retrieve(
    Record :: veon_pdu_movie_retrieve_req:pdu(),
    Req :: cowboy_req:req(),
    State :: state()
) ->
    Ret :: {ok, Req :: cowboy_req:req(), State :: state()}.
...
handle_2_retrieve(Record, Req, State) ->
    Worker = veon_shard:worker_by_id(veon_shard:id_for(veon_pdu_movie_retrieve_req:imdb_id(Record))),
    case veon_worker:lookup_movie(
        Worker,
        veon_pdu_movie_retrieve_req:imdb_id(Record),
        veon_pdu_movie_retrieve_req:screen_id(Record)
    ) of
        {ok, Movie} ->
...
handle_3_generate_response(Movie, Req, State) ->
    ...
    Response = veon_pdu_movie_retrieve_res:new(
        true, Code, Slogan,
...
-spec handle_4_validate_response(
    Record :: veon_pdu_movie_retrieve_res:pdu(),
    Req :: cowboy_req:req(),
    State :: state()
) ->
    Ret :: {ok, Req :: cowboy_req:req(), State :: state()}.
...
handle_4_validate_response(Record, Req, State) ->
    {ok, Document} = veon_pdu_movie_retrieve_res:to_document(Record),
    case veon_pdu_movie_retrieve_res:validate(Document) of
        {ok, Document1} ->
            handle_5_reply(Document1, Req, State);
        {error, Errors} ->
            lager:error("Error validating movie retrieve response: ~p", [Errors]),
...
handle_5_reply(Document, Req, State) ->
    {ok, Req1} = veon_web:ok(Document, Req),
    {ok, Req1, State}.
